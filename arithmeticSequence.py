...
def arithmeticSequencesCommon(A: list,n:int) -> list:
    SubSequence = []
    ArithmeticSequences = []
...
def arithmeticSequences(A: list) -> list:
    SubSequence = []
    ArithmeticSequences = []
    #Create array of pairs from array A
    for index,item in enumerate(A[:-1]):
    ...
    #finding arithmetic sequences
    for index,pair in enumerate(SubSequence):
        for n in range(1,ceil((A[-1]-A[0])/2)+1):
            if (pair[1] - pair[0] == n):
                found = [pair[0],pair[1]]
                for index2,pair2 in enumerate(SubSequence[index+1:]):
                    if (pair2[0] == found[-1] and pair2[1]-pair2[0] == n):
...
def distArSeq(A: list) -> list:
    DistinctArithmeticSequences = arithmeticSequences(A)
...
def distArSeqCommon(A: list,n:int) -> list:
    DistinctArithmeticSequences = arithmeticSequencesCommon(A,n)
    for index,item in enumerate(A):
        for index2,item2 in enumerate([x for x in A if x != item]):
            if (set(item2) <= set(item)):
                DistinctArithmeticSequences.remove(item2)
...
def main():
    ...
    arseq = arithmeticSequences(df,common_difference)
    print(arseq)
